...
```sh
# 1. Create local configuration and secrets (all are ignored by Git).
cp accounts.example.json accounts.json
cp .env.example .env
install -m 600 /path/to/enable-banking-private-key.pem private.pem

# 2. Edit .env: replace every CHANGEME value and set the public HTTPS callback.
# The connector must be able to modify accounts.json after the OAuth callback.
sudo chown 10001:10001 accounts.json private.pem

# 3. Validate and start the main stack.
docker compose config -q
docker compose up -d --build
docker compose ps
docker compose logs -f bank-connector
```

The scheduler runs every 6 hours in Compose. `POST /sync/refresh`, `POST /sync`, and `GET /sync/status` require `X-Sync-Token`.

```sh
# Load the secret from .env without printing it, then verify the service.
set -a; . ./.env; set +a
curl -i -X POST http://127.0.0.1:3000/sync/refresh \
  -H "X-Sync-Token: $BANK_CONN_SYNC_TOKEN"
curl -s http://127.0.0.1:3000/sync/status \
  -H "X-Sync-Token: $BANK_CONN_SYNC_TOKEN"
```

Open the public connector URL to begin the `/connect` OAuth flow. Run a refresh twice and verify in Actual that the second pass does not create duplicates. A 401/403 from Enable Banking is reported as `reauthorization_required`; restart the interactive `/connect` flow rather than retrying aggressively. Start the optional MCP service only when it has been configured: `docker compose --profile mcp up -d actual-mcp`.

## MCP opzionale per Actual Budget

Il servizio `actual-mcp` è disabilitato per impostazione predefinita tramite il profilo Compose `mcp`. Quando viene avviato, usa la rete Docker interna e raggiunge il servizio `actual` a `http://actual:5006`; espone SSE solo su `127.0.0.1:3001` ed è protetto da bearer token.

Compilare anche `ACTUAL_BUDGET_SYNC_ID` e `ACTUAL_MCP_TOKEN` in `.env`, quindi avviarlo e verificarlo:

```sh
docker compose --profile mcp up -d actual-mcp
docker compose logs -f actual-mcp
curl -i http://127.0.0.1:3001/ -H "Authorization: Bearer $ACTUAL_MCP_TOKEN"
```

Per un client MCP compatibile usare l'endpoint SSE `http://127.0.0.1:3001` e il bearer token. Prima di abilitare funzionalità di scrittura, verificare budget e sync ID tramite una richiesta di sola lettura.
